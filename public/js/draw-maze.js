define(["jquery"], function($) {
    ...
    r.getCanvas = function(options) {
        return document.getElementById("myCanvas");
    };

    var drawGrid = r.drawGrid = function(context, options) {
    ...
    var drawBase = r.drawBase = function(context, options) {
    ...
    r.drawWalls = function(context, options, arcOffset) {
        context.beginPath();
    ...
    r.writeStartEnd = function (ctx, options) {
        var x = options.offset - 2;
        var y = options.y * options.bs + 36;
        ctx.font = "12pt Arial";
    ...
    var drawArcs = r.drawArcs = function (options) {
    ...
    var coverArcs = r.coverArcs = function(options) {
        options.players.forEach( function(player) {
           coverArc(player.ctxc, player.coords, options);
        });
    };

    var colors = ['blue', 'orange', 'red', 'green', 'yellow', "pink", "black", "brown"];

    r.initArcs = function (players, room, canvas) {
        var that = this;
        players.forEach( function(player, i) {
            player.ctx = that.getCanvas().getContext("2d");
            player.ctxc = that.getCanvas().getContext("2d");
            player.coords = {
                x: room.offset + room.bs / 2,
                y: room.offset + room.y * room.bs - room.bs /2
            };
    ...
    var count = 0;
    var moveArc = r.moveArc = function(key, offsObj, func, options, ctx, ctx2, player) {
        var move = false;
